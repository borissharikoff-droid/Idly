  ...
  return (
    <div className="rounded-2xl border overflow-hidden" style={{ borderColor: `${cfg.color}28`, background: `${cfg.color}06` }}>
      <div className="px-4 py-3 border-b" style={{ borderColor: `${cfg.color}15` }}>
        ...
        <p className="text-[9px] text-gray-500 font-mono mt-0.5">{cfg.name} — invite friends to join</p>
      </div>
        ...
        {friends.map((f) => {
          ...
          return (
            <div key={f.id} className="flex items-center gap-2.5 px-2 py-1.5 rounded-xl bg-white/[0.03]">
              ...
              {alreadyIn ? (
                <span className="text-[9px] text-green-400 font-mono shrink-0">In party</span>
              ) : alreadySent ? (
                <span className="text-[9px] text-gray-500 font-mono shrink-0">Invited</span>
              ) : (
                <button
                  type="button"
